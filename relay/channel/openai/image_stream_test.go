...
func TestImageExpressionUsesCompletedCountAndProtectsAbortedStreams(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// The synchronous JSON handler keeps Vancine's R16 fail-closed contract,
		// so every non-stream body must carry a real image payload. The PNG
		// magic prefix is this suite's convention for "a valid image".
		{"JSON uses actual count", `{"data":[{"b64_json":"iVBORw0KGgoAAAANSUhEUg"},{"b64_json":"iVBORw0KGgoAAAANSUhEUg"}]}`, false, false, 3, 2},
		{"JSON wrapped as SSE uses actual count", `{"data":[{"b64_json":"first"}]}`, true, false, 3, 1},
		// No "empty response retains request" case: an empty data array must
		// fail closed rather than settle on the requested count. That contract
		// is asserted by TestOpenaiImageHandlerRejectsEmptyOrUnusableResults.
	...
	for _, mode := range []string{"image_generation", "image_edit"} {
		for _, stream := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/stream=%t", mode, stream), func(t *testing.T) {
				// The non-stream branch goes through the R16 fail-closed handler,
				// so the body must carry a real image, not a placeholder string.
				body := `{"data":[{"b64_json":"iVBORw0KGgoAAAANSUhEUg"}],"usage":` + usageJSON + `}`
				contentType := "application/json"
				if stream {
