...
/**
 * Lazy i18n resource loader for the Default theme.
 *
 * Replaces the former static `import` of all 7 locale JSON files in
 * `config.ts` (~2.3MB of translation data bundled into the entry chunk).
 * Each language is mapped to a dynamic `import()` thunk so the bundler
 * emits one chunk per locale file and the first paint only pays for the
 * active language (plus the `en` fallback preloaded by i18next).
 *
 * The module is deliberately side-effect free (no top-level i18next
 * interaction, no DOM access, pure `import()`), so `read` also works in
 * Node/no-`document` environments.
 */
