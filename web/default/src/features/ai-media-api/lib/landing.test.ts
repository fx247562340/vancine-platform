...
*/
// Run with: node --test src/features/ai-media-api/lib/landing.test.ts
...
describe('default ai-media-api landing contract', () => {
  ...
  test('canonical URL is stable across languages', () => {
    assert.equal(
      getAiMediaMetadata('zh').canonical,
  ...
  test('Chinese metadata is selected for bare zh', () => {
    assert.equal(
      getAiMediaMetadata('zh').title,
      '面向开发者的中国 AI 多媒体 API | Vancine'
    )
  })
  ...
  test('docs URLs never point at the upstream docs domain or /docs/api', () => {
    for (const section of [undefined, 'image', 'video', 'speech'] as const) {
...
describe('default ai-media-api locale parity (all locales carry FAQ keys)', () => {
