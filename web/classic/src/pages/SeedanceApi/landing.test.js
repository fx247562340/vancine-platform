...
describe('classic seedance-api landing contract', () => {
  ...
  test('postman collection URL is the exact verified workspace', () => {
    ...
    // Must not carry a query string, fragment, or any credential.
  ...
  test('postman resource tracking payload is exactly the approved shape', () => {
    ...
    // The payload must stay consistent with the shared contract constants.
    assert.equal(SEEDANCE_POSTMAN_TRACKING.event, SEEDANCE_RESOURCE_EVENT);
    assert.ok(
      SEEDANCE_RESOURCE_VALUES.includes(SEEDANCE_POSTMAN_TRACKING.resource),
      'postman resource must be an allowed resource value',
    );
    assert.ok(
      SEEDANCE_RESOURCE_LOCATIONS.includes(SEEDANCE_POSTMAN_TRACKING.location),
  ...
  test('cURL polls at most 120 times and never the old 60 default', () => {
    const shell = SEEDANCE_CODE_EXAMPLES.find((e) => e.id === 'curl');
    // Loop, comment, and timeout message must all use the unified 120 bound.
    ...
    // No legacy 60 default may remain anywhere in the shell example.
