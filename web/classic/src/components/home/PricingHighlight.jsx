  ...
  const pricingData = [
    {
      ...
      title: t('文本生成'),
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: 'var(--vc-text-subtle)' }}>
    ...
    <section style={{ padding: '96px 24px' }}>
      <ScrollReveal>
        <div style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 700, color: 'var(--vc-text-strong)', marginBottom: '16px', lineHeight: 1.2 }}>
            {t('国产模型，全球性价比')}
          </h2>
          <p style={{ color: 'var(--vc-text-muted)', maxWidth: '560px', margin: '0 auto', fontSize: '15px', lineHeight: 1.7 }}>
          ...
          <ScrollReveal key={group.category} delay={idx * 0.15}>
            <div style={{
              ...
              <div style={{
                ...
                {group.icon}
                <span style={{ fontSize: '12px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--vc-text-subtle)' }}>
                  {group.category}
                </span>
                <span style={{ color: 'var(--vc-text-strong)', fontWeight: 500, marginLeft: '4px' }}>{group.title}</span>
              </div>
              ...
              {group.items.map((item, i) => (
                <div key={i} style={{
                  ...
                  gap: '20px',
                  flexWrap: 'wrap',
                  borderTop: i > 0 ? '1px solid var(--vc-border)' : 'none',
                }}>
                  {/* Overseas Model */}
                  <div style={{ flex: '1 1 160px' }}>
                    <div style={{ fontSize: '12px', color: 'var(--vc-text-subtle)', marginBottom: '4px' }}>{t('海外模型')}</div>
                    <div style={{ color: 'var(--vc-text-body)', fontWeight: 500, fontSize: '15px' }}>{item.foreign.name}</div>
                    <div style={{ color: 'var(--vc-text-subtle)', fontSize: '14px', marginTop: '4px' }}>
                      {item.foreign.price}
                  ...
                  {/* Arrow */}
                  <div style={{ color: 'var(--vc-text-subtle)', display: 'flex', flexShrink: 0 }}>
                  ...
                  <div style={{ flex: '1 1 160px' }}>
                    ...
                    <div style={{ color: 'var(--vc-text-strong)', fontWeight: 600, fontSize: '15px' }}>{item.domestic.name}</div>
                    <div style={{ fontSize: '14px', marginTop: '4px' }}>
                      <span style={{ color: 'var(--vc-text-strong)' }}>{item.domestic.price}</span>
                      <span style={{ color: 'var(--vc-text-subtle)', fontSize: '12px' }}>{item.domestic.unit}</span>
                    </div>
                  </div>
      ...
      <ScrollReveal delay={0.5}>
        <p style={{ textAlign: 'center', color: 'var(--vc-text-subtle)', fontSize: '13px', marginTop: '32px' }}>
