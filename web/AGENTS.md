...
| 项目       | 约定                                                                                                                        |
| ---------- | --------------------------------------------------------------------------------------------------------------------------- |
| 视觉体系   | Base Nova + Base UI + Hugeicons；图标一律使用 Hugeicons，不自绘 SVG、不用 emoji 顶替正式图标                                |
| 运行时前端 | 仅 Default 前端（`web/src`）；Default light / Default dark 是同一套前端的两种 appearance                                    |
| 验收基准   | Default light 为主要设计验收基准；Default dark 必须通过现有主题 token 正常适配                                              |
| 主题 token | 颜色、边框、背景、状态一律使用现有主题 token（`--background`/`--card`/`--primary`/`--muted` 等），禁止硬编码页面主题        |
| Classic    | Classic 仅是历史代码与历史文档背景，不是当前可选主题；禁止把 Classic 的纯黑底、重紫色光晕、重玻璃拟态基调当成新页面默认风格 |
| 视觉重构   | 视觉重构必须冻结业务契约：不改出站请求体、表单状态源、能力解析、资源校验、密钥生命周期与任务轮询，只允许改表现层            |
...
- **2026-08-24**：新增「当前视觉基线」：Base Nova + Base UI + Hugeicons；Default light 为主要验收基准、Default dark 必须适配；仅使用主题 token；视觉重构冻结业务契约。
