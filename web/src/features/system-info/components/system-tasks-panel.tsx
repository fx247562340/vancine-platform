...
const SYSTEM_TASK_SKELETON_KEYS = [
  'system-task-skeleton-slot-01',
  'system-task-skeleton-slot-02',
  'system-task-skeleton-slot-03',
  'system-task-skeleton-slot-04',
] as const
...
export function SystemTasksPanel() {
  ...
  if (loading) {
    bodyContent = (
      <div className='space-y-2 p-4 sm:p-5'>
        {SYSTEM_TASK_SKELETON_KEYS.map((skeletonKey) => (
          <Skeleton key={skeletonKey} className='h-9 w-full rounded-md' />
        ))}
      </div>
    )
  ...
  return (
    <section className='bg-card overflow-hidden rounded-lg border shadow-xs'>
      ...
      <div aria-busy={tasksQuery.isFetching}>{bodyContent}</div>
    </section>
  )
}
