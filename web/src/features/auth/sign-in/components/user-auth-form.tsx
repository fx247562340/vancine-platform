...
export function UserAuthForm({
  ...
  async function handleWeChatLogin() {
    ...
    try {
      ...
      if (res?.success) {
        // Vancine Google Ads attribution: a WeChat login from the sign-in page
        // can still durably create a brand-new account server-side. Only the
        // server-confirmed flag (never an existing-user login, and never a
        // pending login challenge) triggers the signup conversion; the bundle's
        // user id is the local-only per-signup dedup key and is never sent to
        // Google.
