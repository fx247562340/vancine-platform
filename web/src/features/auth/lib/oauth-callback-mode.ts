...
*/
/**
 * Tells apart the OAuth callbacks that land on the same `/oauth/:provider`
 * route: an account **bind** or a security **verify**, which run inside a popup
 * we opened, and a plain **login** redirect, which runs in the user's own tab.
 *
 ...
 *
 * The popup we open is same-origin (`about:blank`) before it is sent to the
 * provider, so we stamp its own sessionStorage. That stamp rides along through
 * the provider round trip and is scoped to the popup alone, which makes it
 * positive proof of a bind/verify flow.
 */
