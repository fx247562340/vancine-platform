...
it('uses backend square states for success, warning, hidden, and partial rows including models without metadata', async () => {
  ...
  // Resolve the expected labels through the same zh resource bundle this file
  // already loads (`zhCN.translation`, added in beforeEach) instead of pinning
  // upstream's literals: Vancine deliberately ships its own zh product copy for
  // some keys — `Unavailable` is 暂不可用 here, not upstream's 无法展示, and that
  // value is itself asserted by
  // src/features/acquisition/admin/__tests__/funnel-i18n.test.ts. The deployed
  // bundle is the source of truth, so the test reads it rather than restating it.
  const zhCopy: Record<string, string> = {
    Unavailable: i18n.t('Unavailable'),
    'Partly shown': i18n.t('Partly shown'),
    Displayed: i18n.t('Displayed'),
    'Listing hidden': i18n.t('Listing hidden'),
  }
  // Guard: each lookup must actually be localized. Without it, a missing bundle
  // would make every assertion below pass vacuously against the English source.
  for (const [source, label] of Object.entries(zhCopy)) {
    expect(label).not.toBe(source)
  }
  expect(screen.getByText(zhCopy.Unavailable)).toBeVisible()
  expect(screen.getByText(zhCopy['Partly shown'])).toBeVisible()
  expect(screen.getAllByText(zhCopy.Displayed)).toHaveLength(2)
  expect(screen.getAllByText(zhCopy['Listing hidden'])).toHaveLength(2)
  for (const label of [zhCopy.Unavailable, zhCopy['Partly shown']]) {
