...
import { AI_MEDIA_I18N_KEYS } from '../landing'

/**
 * Locale completeness contract for the AI Media landing page and the shared
 * developer solutions registry: every key must exist, be a real value, and
 * keep the same interpolation placeholders in all seven supported locales.
 */
...
const CONTRACT_KEYS: readonly string[] = [
  ...
  DEVELOPER_SOLUTIONS_MENU_LABEL_KEY,
  DEVELOPER_SOLUTIONS_SECTION_LABEL_KEY,
  ...DEVELOPER_SOLUTIONS.flatMap((solution) => [
    solution.titleKey,
...
/** Product names that legitimately stay identical across locales. */
const PROPER_NOUN_KEYS = new Set(['Kimi K3 API', 'AI Media API'])

describe('ai-media-api locale completeness', () => {
  test('every contract key exists and is a real value in all seven locales', () => {
  ...
  test('sentence-level copy is translated, not copied from English', () => {
    const sentenceKeys = CONTRACT_KEYS.filter(
      (key) => key.length >= 20 && !PROPER_NOUN_KEYS.has(key)
    )
