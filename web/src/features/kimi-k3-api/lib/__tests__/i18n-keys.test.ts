...
import { KIMI_K3_I18N_KEYS } from '../landing'

/**
 * Locale completeness contract for the Kimi K3 landing page: every key the
 * page passes to t() must exist, be a real translation, and keep the same
...
describe('kimi-k3-api locale completeness', () => {
  test('every page key exists and is a real value in all seven locales', () => {
    for (const key of KIMI_K3_I18N_KEYS) {
  ...
  test('sentence-level copy is translated, not copied from English', () => {
    const sentenceKeys = KIMI_K3_I18N_KEYS.filter((key) => key.length >= 20)
