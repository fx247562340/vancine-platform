...
const CREEM_PRODUCT_SKELETON_KEYS = [
  'creem-product-skeleton-plan',
  'creem-product-skeleton-credit',
  'creem-product-skeleton-pay',
] as const
  ...
  if (loading) {
    return (
      <div className='grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-3 md:grid-cols-3'>
        {CREEM_PRODUCT_SKELETON_KEYS.map((skeletonKey) => (
          <Skeleton key={skeletonKey} className='h-24 rounded-lg' />
        ))}
      </div>
    )
  }
