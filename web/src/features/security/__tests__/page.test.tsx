...
describe('security page migration', () => {
  ...
  it('built-in and custom bindings share one compact responsive grid', async () => {
    ...
    // Every action button - built-in and custom alike - carries its provider in
    // the accessible name, because the visible text is only the action and
    // several rows would otherwise render an identical 'Bind'. getByRole also
    // proves each name is unique within the list.
    expect(
      within(bindings).getByRole('button', { name: 'Bind Gitea' })
    ).toBeVisible()
    expect(
      within(bindings).getByRole('button', { name: 'Bind Email' })
    ).toBeVisible()
    expect(
      within(bindings).getByRole('button', { name: 'Bind GitHub' })
    ).toBeVisible()
    expect(
      within(bindings).getByRole('button', { name: 'Bind OIDC' })
    ).toBeVisible()
    expect(
      within(bindings).queryByRole('button', { name: 'Bind' })
