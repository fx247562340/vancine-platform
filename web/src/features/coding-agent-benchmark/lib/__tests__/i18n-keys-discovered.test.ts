...
function discoverLiterals(): string[] {
  ...
  for (const file of SOURCE_FILES) {
    ...
    if (rel === 'lib/coding-agent-benchmark.ts') continue
...
describe('coding-agent-benchmark source-driven i18n', () => {
  test('every t() literal in components is registered', () => {
    assert.ok(DISCOVERED.length > 0)
    const registry = new Set<string>(CODING_AGENT_BENCHMARK_I18N_KEYS)
