    ...
    void (async () => {
      try {
        ...
        if (response.data?.success) {
          ...
          // OAuth account (not an existing-user login, and not a pending login
          // challenge) triggers the signup conversion; bind popups never reach
          // this branch. The bundle's user id is the local-only per-signup
          // dedup key and is never sent to Google.
          if (
            isAuthBundle(response.data.data) &&
            isServerConfirmedNewUser(response.data.data)
          ) {
