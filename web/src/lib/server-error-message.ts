...
export function getServerErrorMessage(
  ...
  for (const source of sources) {
    // An axios error that received a response only carries axios's own
    // synthesized transport text here ("Request failed with status code 500"),
    // an empty body, or a proxy's HTML error document — never an actionable API
    // message. Anything the server actually said was already resolved from the
    // response body above, so letting the transport text win would make every
    // caller's localized fallback unreachable. A response-less axios error
    // ("Network Error", timeouts) keeps its message: it is the only diagnostic.
    if (source.isAxiosError && isRecord(source.response)) continue
