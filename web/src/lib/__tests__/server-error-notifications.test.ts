...
// ---------------------------------------------------------------------------
// Real AxiosError regressions: a caller's localized fallback must survive the
// response interceptor, and a genuine server message must still outrank it.
// ---------------------------------------------------------------------------

/**
 * Reject one real request through the real client so the production response
 * interceptor in lib/http-client.ts runs on a real AxiosError, then hand back
 * the exact error object a caller's catch block would receive.
 */
async function rejectWithBody(
  data: unknown,
  status = 500
): Promise<AxiosError> {
  api.defaults.adapter = async (config) => {
  ...
  return failure as AxiosError
}

it.each([
  ['an empty body', {}],
  [
    'an HTML proxy error document',
    '<!doctype html><html><body>502 Bad Gateway</body></html>',
  ],
  ['a body whose message is only whitespace', { message: '   ' }],
  [
    'a body whose message is an HTML proxy error document',
    { message: '<html><body>502 Bad Gateway</body></html>' },
  ],
])(
  'reports the caller fallback exactly once when a real 500 AxiosError carries %s',
  async (_label, data) => {
    const notify = vi.spyOn(toast, 'error').mockReturnValue('error')
    const error = await rejectWithBody(data)
    expect(error).toBeInstanceOf(AxiosError)
    expect(getServerErrorMessage(error, 'Operation failed')).toBe(
      'Operation failed'
    )
    handleServerError(error, 'Operation failed')
    handleServerError(error, 'Operation failed')
    expect(notify.mock.calls.map(([message]) => message)).toEqual([
      'Operation failed',
    ])
...
it('keeps the transport status code in error.message for logging without ever showing it to the user', async () => {
  const notify = vi.spyOn(toast, 'error').mockReturnValue('error')
  const error = await rejectWithBody({})
  // The interceptor must not fabricate a presentation message, so the honest
  // axios text stays available to logs while the caller's fallback wins in UI.
...
it('keeps an explicit backend message ahead of the caller fallback on a real 500 AxiosError', async () => {
  const notify = vi.spyOn(toast, 'error').mockReturnValue('error')
  const error = await rejectWithBody({
    success: false,
...
it('resolves a stable backend error code to the localized message instead of the caller fallback', async () => {
  i18next.addResourceBundle('zhCN', 'translation', zhCN.translation, true, true)
  await i18next.changeLanguage('zhCN')
  const notify = vi.spyOn(toast, 'error').mockReturnValue('error')
  const error = await rejectWithBody({ code: 'TWOFA_CODE_INVALID' }, 400)
  const expected = zhCN.translation['The authenticator code is incorrect.']
  // Anti-vacuity: the shipped zh bundle must really translate this key, so the
  // assertion cannot pass against an unloaded bundle returning the en source.
  expect(expected).toBe('认证器验证码错误。')
  expect(expected).not.toBe('The authenticator code is incorrect.')
  handleServerError(error, 'Operation failed')
  handleServerError(error, 'Operation failed')
