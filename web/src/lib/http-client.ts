...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (axios.isAxiosError(error)) {
      // Only adopt a message the server actually provided. Rewriting
      // error.message with generic transport text would both discard the real
      // status code for logging and outrank the caller's localized fallback.
...
api.interceptors.request.use(async (config) => {
  ...
  config.headers['Accept-Language'] = toLanguageTag(i18n.language)
  return config
})
