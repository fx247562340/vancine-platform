...
// newSecuritySQLiteDatabase opens an isolated temp-file SQLite database whose
// DSN mirrors common.SQLitePath's production transaction semantics exactly:
// a 30s busy timeout so concurrent writers queue instead of failing with
// SQLITE_BUSY, WAL so readers never block the single writer, and BEGIN
// IMMEDIATE (_txlock=immediate) so a read-then-write transaction takes the
// write lock up front instead of dying on SQLITE_BUSY_SNAPSHOT — the exact
// failure the concurrent account-deletion regression pins down.
//
// Scope: the security enrollment / account-deletion suites only. The generic
// newAuditTestDatabase fixture must NOT use BEGIN IMMEDIATE: some upstream
// concurrency tests (e.g. TestManageUserQuotaConcurrentSnapshots) model "both
// transactions reach the query barrier first", which requires deferred
// transaction starts; a shared immediate-txlock fixture would deadlock them.
// This is test-fixture isolation, not a production behavior fork —
// common.SQLitePath itself is untouched and no production code differs.
func newSecuritySQLiteDatabase(t *testing.T) *gorm.DB {
	t.Helper()
	db, err := gorm.Open(
		sqlite.Open(t.TempDir()+"/security.db?_pragma=busy_timeout(30000)&_pragma=journal_mode(WAL)&_txlock=immediate"),
		&gorm.Config{},
	)
...
func setupSecurityEnrollmentTest(t *testing.T) (*model.User, service.AuthIdentity) {
	...
	if dialect == "sqlite" {
		// Only the security suites verify production SQLite concurrency
		// semantics (concurrent account deletion), so only they use the
		// production-DSN fixture; every other consumer of the generic fixture
		// keeps upstream's plain deferred-transaction semantics.
		db = newSecuritySQLiteDatabase(t)
		logDB = newSecuritySQLiteDatabase(t)
	} else {
	...
	*system_setting.GetPasskeySettings() = system_setting.PasskeySettings{Enabled: true, RPID: "example.com", Origins: "https://example.com", RPDisplayName: "new-api"}
	t.Cleanup(func() {
		...
		*system_setting.GetPasskeySettings() = previousSettings
		...
		if logDB != db {
			if connection, err := logDB.DB(); err == nil {
				_ = connection.Close()
			}
		}
	})
