...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "currency amount below int32 still converts",
			displayType: operation_setting.QuotaDisplayTypeUSD,
			amount:      4294,
			wantQuota:   2_147_000_000,
		},
		{
			name:        "currency amount above MaxQuota is a legal wallet credit",
			displayType: operation_setting.QuotaDisplayTypeUSD,
			amount:      4295,
		...
		},
		{
			...
			amount:      2_147_000_000,
			wantQuota:   2_147_000_000,
		},
		{
			name:        "token amount above MaxQuota is a legal wallet credit",
			displayType: operation_setting.QuotaDisplayTypeTokens,
...
func TestValidateCreditedQuotaRejectsOverflow(t *testing.T) {
	...
	_, err = validateCreditedQuota(decimal.NewFromInt(int64(common.MaxWalletQuota) + 1))
	require.EqualError(
		t,
		err,
		"充值额度超出系统可表示范围",
...
func TestStripeCreditedQuotaIncludesGroupRatio(t *testing.T) {
	...
	_, err := validateCreditedQuota(getStripeCreditedQuota(1073, "vip"))
	require.NoError(t, err)
	_, err = validateCreditedQuota(getStripeCreditedQuota(2148, "vip"))
	require.NoError(t, err, "2148 * 2x ratio * 500000 is above MaxQuota and still in the wallet domain")
