...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"paypal_min_topup":        getPayPalMinTopup(),
...
func EpayNotify(c *gin.Context) {
	...
	// 只读预检，仅用于保留可运维的告警日志；订单存在性、支付网关和 pending
	// 状态的权威校验在下面的事务里完成。
	if preCheck := model.GetTopUpByTradeNo(verifyInfo.ServiceTradeNo); preCheck == nil {
	...
	} else if preCheck.PaymentProvider != model.PaymentProviderEpay {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 订单支付网关不匹配 trade_no=%s order_provider=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, preCheck.PaymentProvider, verifyInfo.Type, c.ClientIP()))
	} else if preCheck.Status == common.TopUpStatusPending && preCheck.PaymentMethod != verifyInfo.Type {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 实际支付方式与订单不同，按实际支付方式记账 trade_no=%s order_payment_method=%s actual_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, preCheck.PaymentMethod, verifyInfo.Type, c.ClientIP()))
	}

	// 订单状态与额度增加（含首次充值赠送）在 model 层的同一个事务里完成。
	// 本进程锁只减少同一订单的重复处理，跨实例、跨渠道的幂等靠事务行锁保证。
	if err := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值结算失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), err.Error()))
		// 不确认成功，让上游重试；已经充值成功的订单会走幂等空操作分支。
		if _, err := c.Writer.Write([]byte("fail")); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), err.Error()))
		}
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值处理完成 trade_no=%s client_ip=%s", verifyInfo.ServiceTradeNo, c.ClientIP()))
	if _, err := c.Writer.Write([]byte("success")); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 webhook 响应写入失败 trade_no=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, c.ClientIP(), err.Error()))
	}
}
