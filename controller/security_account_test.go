...
func TestSecurityAccountPasswordRequiresCurrentPassword(t *testing.T) {
	// Pin the storage format explicitly: Vancine's default write is bcrypt, and
	// this case asserts the Argon2id write path, so it must opt in rather than
	// rely on the ambient default.
...
func TestSecurityAccountLongUnicodePasswordAndSessionRotation(t *testing.T) {
	// Long passwords are an Argon2id-only capability: Vancine's default write
	// algorithm is bcrypt, which refuses anything over 72 bytes, so this case
	// opts in explicitly instead of relying on the ambient default.
	t.Setenv("ACCOUNT_PASSWORD_HASH_ALGORITHM", "argon2id")
	user, identity := setupSecurityEnrollmentTest(t)
...
func TestSecurityAccountEncryptedLongPasswordLogin(t *testing.T) {
	// Same reason as above: the 128-emoji password needs the Argon2id opt-in.
	t.Setenv("ACCOUNT_PASSWORD_HASH_ALGORITHM", "argon2id")
	user, identity := setupSecurityEnrollmentTest(t)
