...
func handleOAuthBind(c *gin.Context, providerName string, provider oauth.Provider, oauthUser *oauth.OAuthUser, flow *model.AuthFlow, state string, match model.AuthFlowMatch) (bool, bool) {
	...
	// Google is skipped here: its ownership lives in external_identity_claims
	// and is enforced atomically inside the bind transaction below, never by a
	// check-then-write on the mirror column.
	_, isGoogle := provider.(*oauth.GoogleProvider)
	if !isGoogle {
	...
	_, err = model.ConsumeAuthFlowWithAction(state, match, func(tx *gorm.DB, _ *model.AuthFlow) error {
		if isGoogle {
			// Vancine: revalidate the session under the transaction's locks, then
			// commit the durable claim and the google_sub mirror as one unit. The
			// claim's unique indexes reject a subject owned by another user and any
			// rebind by an already-bound user, including concurrent attempts.
			if err := model.ValidateAuthSessionWithTx(tx, identity); err != nil {
				return err
			}
			return model.BindGoogleIdentityWithTx(tx, strings.TrimSpace(oauthUser.ProviderUserID), identity.UserID)
		}
		if providerName == "telegram" {
