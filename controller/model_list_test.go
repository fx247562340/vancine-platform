...
func TestGetUserModelsFiltersChatEligibleModelsByModality(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&model.User{
		Id:       1010,
		Username: "playground-chat-modality-user",
		Password: "password",
		Group:    "default",
		Status:   common.UserStatusEnabled,
	}).Error)
	require.NoError(t, db.Create(&[]model.Ability{
		...
		{Group: "default", Model: "kimi-k2.5", ChannelId: 1, Enabled: true},
		// Image
		{Group: "default", Model: "qwen-image-2.0-pro", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "qwen-image-3.0", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "qwen-image-3.0-pro", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "Doubao-Seedream-5.0-pro", ChannelId: 1, Enabled: true},
		// Video
		{Group: "default", Model: "Doubao-Seedance-1.5-pro", ChannelId: 1, Enabled: true},
		// TTS
		{Group: "default", Model: "Doubao-tts", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "mimo-v2.5-tts", ChannelId: 1, Enabled: true},
		// 3D
		{Group: "default", Model: "Doubao-Seed3D-2.0", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "Hitem3D-2.0", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "Hyper3D-Gen2", ChannelId: 1, Enabled: true},
	}).Error)

	// 1) modality=chat must strip every media family but keep all chat
	//    models, including ones the previous image-only filter missed.
	chatRecorder := httptest.NewRecorder()
	chatContext, _ := gin.CreateTestContext(chatRecorder)
	chatContext.Request = httptest.NewRequest(http.MethodGet, "/api/user/models?group=default&modality=chat", nil)
	chatContext.Set("id", 1010)
	GetUserModels(chatContext)
	chatModels := decodeUserModelsResponse(t, chatRecorder)
	assert.ElementsMatch(t, []string{"gpt-4o", "kimi-k2.5"}, chatModels)

	// 2) Without modality the original interface contract is preserved:
	//    every enabled ability is returned, including media-only models.
	noModalityRecorder := httptest.NewRecorder()
	noModalityContext, _ := gin.CreateTestContext(noModalityRecorder)
	noModalityContext.Request = httptest.NewRequest(http.MethodGet, "/api/user/models?group=default", nil)
	noModalityContext.Set("id", 1010)
	GetUserModels(noModalityContext)
	noModalityModels := decodeUserModelsResponse(t, noModalityRecorder)
	assert.Len(t, noModalityModels, 12)

	// 3) Unknown modality value must be a no-op (case-insensitive) so
	//    future clients can pass new values without breaking older builds.
	unknownRecorder := httptest.NewRecorder()
	unknownContext, _ := gin.CreateTestContext(unknownRecorder)
	unknownContext.Request = httptest.NewRequest(http.MethodGet, "/api/user/models?group=default&modality=video", nil)
	unknownContext.Set("id", 1010)
	GetUserModels(unknownContext)
	assert.ElementsMatch(t, noModalityModels, decodeUserModelsResponse(t, unknownRecorder))
}

func TestGetUserModelsChatModalityAcceptsUnknownFutureModels(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&model.User{
		Id:       1011,
		Username: "playground-chat-future-user",
		Password: "password",
		Group:    "default",
		Status:   common.UserStatusEnabled,
	}).Error)
	require.NoError(t, db.Create(&model.Ability{
		Group: "default", Model: "brand-new-chat-model-2026", ChannelId: 1, Enabled: true,
	}).Error)

	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Request = httptest.NewRequest(http.MethodGet, "/api/user/models?group=default&modality=chat", nil)
	context.Set("id", 1011)
	GetUserModels(context)

	models := decodeUserModelsResponse(t, recorder)
	assert.Equal(t, []string{"brand-new-chat-model-2026"}, models)
}
