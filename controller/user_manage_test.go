...
func TestManageUserQuotaRespectsWalletCeiling(t *testing.T) {
	...
	user := model.User{
		...
		Status: common.UserStatusEnabled, Group: "default", Quota: common.MaxWalletQuota - 1, AffCode: "aff-wallet-ceil",
	}
	require.NoError(t, db.Create(&user).Error)
	...
	under := model.User{
		Username: "managed-quota-plus-one", Password: "password", Role: common.RoleCommonUser,
		Status: common.UserStatusEnabled, Group: "default", Quota: common.MaxQuota, AffCode: "aff-wallet-plus",
	}
	require.NoError(t, db.Create(&under).Error)
	recorder = performManageUserRequest(t, fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":"add","value":1}`, under.Id))
	assert.Contains(t, recorder.Body.String(), `"success":true`)
	var plusOne model.User
	require.NoError(t, db.First(&plusOne, under.Id).Error)
	assert.Equal(t, common.MaxQuota+1, plusOne.Quota)
}
