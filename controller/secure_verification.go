...
func writeSecurityOperationError(c *gin.Context, err error) {
	...
	case errors.Is(err, model.ErrExternalIdentityAlreadyClaimed):
		...
		} else if provider != nil && provider.GetName() != "" {
			// Vancine: restore the localized, provider-named conflict copy this
			// path carried before the security-core adoption, when
			// handleOAuthBind rendered i18n.MsgOAuthAlreadyBound itself. The
			// stable ACCOUNT_ALREADY_BOUND code above is deliberately unchanged,
			// and an unresolvable slug keeps the generic message.
			message = i18n.T(c, i18n.MsgOAuthAlreadyBound, providerParams(provider.GetName()))
		}
