...
type User struct {
	...
	// Vancine keeps the wallet columns 64-bit: quota, used_quota, aff_quota and
	// aff_history are guarded by ensureUserQuotaColumns and must stay bigint.
...
var userBindColumns = map[string]bool{
	...
	"linux_do_id": true,
	"wechat_id":   true,
	// Vancine Google OAuth binding mirrors into users.google_sub.
	"google_sub": true,
}
...
func (user *User) ClearBinding(bindingType string) error {
	...
	// receiver's mirror field with the committed state, then return. The
	// durable release already read back the persisted state inside the
	// transaction, so no DB reload or cache refresh is needed and a cache
	// outage can never turn a committed admin clear into a reported failure.
	// Non-Google bindings keep the original contract below: UserBase caches
	// fields such as email, so reload and cache errors must still surface.
	if bindingType == ExternalIdentityProviderGoogle {
		user.GoogleSub = ""
		return nil
	}
