...
type TopUp struct {
	...
	// for first-top-up-bonus history; Amount=0 marks a subscription/balance row.
	PaymentProvider string `json:"payment_provider" gorm:"type:varchar(50);default:''"`
	// PaymentId stores the provider-side order identifier (PayPal Order ID).
	PaymentId string `json:"payment_id" gorm:"type:varchar(255);default:'';index"`
	// TransactionId stores the settled provider transaction identifier
	// (PayPal Capture ID); unique so one capture can never settle two
	// orders. Pointer type: pending orders carry SQL NULL, and all three
	// supported databases allow multiple NULLs in a unique index while
	// rejecting duplicate capture ids.
	TransactionId *string `json:"transaction_id" gorm:"type:varchar(255);uniqueIndex"`
	CreateTime    int64  `json:"create_time"`
	CompleteTime  int64  `json:"complete_time"`
	Status        string `json:"status"`
	// BaseQuota records the paid quota the settlement actually credited. The
	// persisted value is authoritative for refunds: a refund must never
	// re-derive the credit from the current QuotaPerUnit. 0 means the row was
	// settled before this field existed. Column width is 64-bit
	// (bigint/int8/INTEGER), identical to the upstream wallet domain.
	BaseQuota int `json:"base_quota" gorm:"type:bigint;default:0"`
	// BonusQuota records the first-top-up bonus granted with this order; 0
	// means no bonus was granted. Refunds claw it back together with
	// BaseQuota. Column width is 64-bit, identical to the wallet domain.
	BonusQuota int `json:"bonus_quota" gorm:"type:bigint;default:0"`
}
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	syncCreditUserQuotaCache(topUp.UserId, bonus, "epay first top-up bonus")
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	syncCreditUserQuotaCache(topUp.UserId, bonus, "stripe first top-up bonus")
...
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	syncCreditUserQuotaCache(topUp.UserId, bonus, "creem first top-up bonus")
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	syncCreditUserQuotaCache(topUp.UserId, bonus, "waffo first top-up bonus")
...
func RechargeWaffoPancake(tradeNo string) (err error) {
	...
	syncCreditUserQuotaCache(topUp.UserId, bonus, "waffo pancake first top-up bonus")
