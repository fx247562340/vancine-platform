...
func channelMatchesFilter(ch *Channel, modelName string, filter dto.ChannelFilter) bool {
	...
	case dto.FilterRequestPath:
		...
		// Vancine Image Workspace: restrict image models on the playground
		// generation seam to their required provider channel type.
		if playground.ApplyImageWorkspaceEligibility(filter.RequestPath, modelName) &&
			!playground.ChannelTypeEligibleForModel(ch.Type, modelName) {
			return false
		}
