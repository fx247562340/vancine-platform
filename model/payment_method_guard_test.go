...
func epayTestTransactionID(tradeNo string) *string { return &tradeNo }
...
func createEpayTestOrder(t *testing.T, userId int, tradeNo string, provider string, status string) TopUp {
	...
	topUp := TopUp{
		...
		TransactionId:   epayTestTransactionID(tradeNo),
...
// TestRechargeEpayConcurrentSettlementChargesExactlyOnce drives two concurrent
// settlement attempts for two orders of the same user at the MaxWalletQuota
// boundary. The atomic conditional credit allows at most one order to win the
// headroom; the other must roll back with ErrTopUpQuotaLimitExceeded while the
// balance stays exactly at the ceiling — no wrap, no double credit.
func TestRechargeEpayConcurrentSettlementChargesExactlyOnce(t *testing.T) {
	oldQuotaPerUnit := common.QuotaPerUnit
	common.QuotaPerUnit = 500000
	t.Cleanup(func() { common.QuotaPerUnit = oldQuotaPerUnit })

	truncateTables(t)
	// Each epay test order (Amount=2) settles to 1,000,000 quota at
	// QuotaPerUnit=500000. Leave headroom for exactly one of them.
	user := insertUserForPaymentGuardTest(t, 601, common.MaxWalletQuota-1_000_000)
	orderA := createEpayTestOrder(t, user.Id, "EPAYCONCURRENCYA", PaymentProviderEpay, common.TopUpStatusPending)
	orderB := createEpayTestOrder(t, user.Id, "EPAYCONCURRENCYB", PaymentProviderEpay, common.TopUpStatusPending)
	...
	for _, order := range []TopUp{orderA, orderB} {
		go func(order TopUp) {
			alreadyDone, err := RechargeEpay(order.TradeNo, "alipay", "127.0.0.1")
			results <- result{tradeNo: order.TradeNo, alreadyDone: alreadyDone, err: err}
		}(order)
	}
	...
	for i := 0; i < 2; i++ {
		...
		if r.err == nil {
			succeeded++
			assert.False(t, r.alreadyDone, "a fresh order must not settle as a replay")
		} else {
			require.ErrorIs(t, r.err, ErrTopUpQuotaLimitExceeded)
			capacityRejected++
		}
	}
	assert.Equal(t, 1, succeeded, "exactly one order must fit the remaining headroom")
	assert.Equal(t, 1, capacityRejected, "the other order must be rejected by the capacity gate")

	assert.Equal(t, common.MaxWalletQuota, getUserQuotaForPaymentGuardTest(t, user.Id))

	// The winning order is success; the losing order must stay pending for
	// gateway retry (it was never settled). Which one wins is nondeterministic
	// — exactly one winning is the invariant under test.
	statusA := getTopUpStatusForPaymentGuardTest(t, orderA.TradeNo)
	statusB := getTopUpStatusForPaymentGuardTest(t, orderB.TradeNo)
	// The settled one may be either order; the other must stay pending.
	assert.True(t,
		(statusA == common.TopUpStatusSuccess && statusB == common.TopUpStatusPending) ||
			(statusB == common.TopUpStatusSuccess && statusA == common.TopUpStatusPending),
		"exactly one of the two orders may settle")
}
