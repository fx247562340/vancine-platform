...
// WalletQuotaFromFloatStrict converts a wallet credit computed in float64
// (truncating toward zero) and rejects a value outside ±MaxWalletQuota.
// Stripe and PayPal keep this truncation contract; only the representable
// range is the wallet domain, not MaxQuota.
func WalletQuotaFromFloatStrict(value float64) (int, error) {
	return strictQuota(saturateQuotaBounded(value, "WalletQuotaFromFloat", MaxWalletQuota, -MaxWalletQuota))
}
