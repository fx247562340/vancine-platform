...
func TestWalletQuotaFromDecimalStrict(t *testing.T) {
	quota, err := WalletQuotaFromDecimalStrict(decimal.NewFromInt(1_000_000_000))
	require.NoError(t, err)
	...
	quota, err = WalletQuotaFromDecimalStrict(decimal.NewFromInt(int64(MaxQuota) + 1))
	require.NoError(t, err)
...
func TestQuotaFromDecimalStrictStillRejectsMaxQuotaPlusOne(t *testing.T) {
	quota, err := QuotaFromDecimalStrict(decimal.NewFromInt(int64(MaxQuota) + 1))
	assert.Zero(t, quota)
	var clamp *QuotaClamp
	require.ErrorAs(t, err, &clamp)
	assert.Equal(t, QuotaClampOverflow, clamp.Kind)
...
func TestValidateWalletQuota(t *testing.T) {
	require.NoError(t, ValidateWalletQuota(MaxQuota+1))
