...
//
// Vancine merge decision (upstream-sync, item E6): the default write stays
// bcrypt for this round and Argon2id is an explicit opt-in via
// ACCOUNT_PASSWORD_HASH_ALGORITHM=argon2id. Reads remain dual-format
// (ValidatePasswordAndHash accepts both), so advancing a deployment later needs
// no data migration and no rewrite of existing bcrypt hashes.
func HashAccountPassword(password string) (string, error) {
	...
	case "argon2id":
		// Explicit opt-in only; falls through to the Argon2id derivation below.
	case "", "bcrypt":
		if len(password) > 72 {
