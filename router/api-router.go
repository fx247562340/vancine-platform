...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/homepage/stats", controller.GetHomepageStats)
		apiRouter.GET("/pi/catalog", controller.GetPiCatalog)
		...
		// Telegram moved to upstream's unified OAuth flow; these legacy paths
		// stay registered so old clients get upstream's explicit deprecation
		...
		// Standard OAuth providers (GitHub, Discord, OIDC, LinuxDO, Telegram, Google) - unified route
		...
		apiRouter.GET("/paypal/return", controller.HandlePayPalReturn)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				// Google self-unbind: fixed narrow route (no binding_type
				// parameter), lockout-protected in the handler.
				selfRoute.DELETE("/self/bindings/google", controller.UnbindGoogleSelf)
			}
