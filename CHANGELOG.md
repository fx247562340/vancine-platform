...
- 修复快速编程模型指南在模型缺少独立图标时显示问号的问题，使用供应商图标作为回退。
- 从表达式计费配置读取输入、输出与缓存价格，避免显示迁移占位价格，并保留旧 Token 计费模型的现有价格展示行为。
...
- 修正模型详情页 API 调用示例的环境变量名称，统一使用 `VANCINE_API_KEY`。

## 2.10.2 - 2026-09-19

### Pi 模型目录

- 修复公开 Pi 模型目录在平台模型迁移到表达式计费后错误排除全部聊天模型的问题，恢复已验证模型在 Pi Agent 中的选择与使用。
- 将可无损映射的单层 Token 计费表达式转换为 Pi 的输入、输出、缓存读取和缓存写入单价，并保持表达式计费优先于残留的旧按次配置。
- 继续排除图片、视频、任务、按次、多档及依赖请求条件的动态表达式，避免向 Pi 发布无法准确表示的价格。

## 2.10.1 - 2026-09-19

### 品牌版本展示

- 从首页和控制台顶部品牌区域移除管理员可见的当前版本号与更新按钮，保持 Logo、系统名称、首页链接和其他导航功能不变。
- 保留“系统设置 → 维护”中的版本检查、更新提示与更新对话框，并继续通过 `/api/status` 提供运行版本信息。

## 2.10.0 - 2026-09-18

### 上游基线升级

- 将 Vancine 基线同步至 new-api upstream/main 固定提交 `7209b6db95ef33bfb0f6344ebf61c1fec9af780c`，引入统一安全验证、访问令牌审计、模型与渠道管理、任务插件及管理后台改进。
- 采用上游最新版 Wan 3.0、任务插件、模型定价、请求转换和 OpenRouter 兼容实现，减少后续跟随上游升级时的长期分叉。
- 保持 relaykit 独立构建能力，并完成根模块、relaykit 与前端完整回归验证。

### 账户与安全

- 接入统一登录验证、安全账户删除、访问令牌管理与审计、多 RP ID Passkey、Telegram OAuth 和强化后的账户绑定流程。
- 保留 Vancine 的 Google OAuth 身份归属与重新绑定保护，并恢复 Provider 名称明确的本地化冲突提示和可访问的绑定操作名称。
- 密码写入继续默认使用 bcrypt，保留 Argon2id 显式启用和双格式读取能力，避免升级后静默切换现有密码策略。

### 管理、计费与插件

- 同步上游渠道、模型、定价、用量日志、审计日志和任务插件管理能力，包括插件市场、模型定价编辑、请求模拟及新的管理界面。
- Wan 3.0 和任务插件的请求、响应与计费行为统一采用上游实现；保留 Vancine 首充奖励、PayPal 扩展及公开定价页的品牌与运营内容。
- 保留 Tencent TokenHub 流式 usage、严格图片计费与结果校验、Seedream 5 等仍有效的 Vancine Provider 行为。

### Vancine 产品兼容

- 保留 Vancine 首页、品牌、文档、SEO、Agent Provider 指南、独立管理员获客漏斗和首方归因链路。
- 保留 Vancine 的 PostgreSQL 生产拓扑、自有数据表与部署流程；MySQL 相关实现跟随上游，不增加 Vancine 专属兼容分支。
- 修正空响应或 HTML 错误的本地化回退、兑换码加载失败提示、账号绑定无障碍名称及 SQLite 安全删除测试夹具作用域。
...
- 为内部管理员获客漏斗提供无第三方统计脚本的独立 SPA shell，响应不生成公开 canonical，并通过 `X-Robots-Tag: noindex, nofollow` 阻止支持该指令的搜索引擎收录。
- 内部路径及其百分号编码等价形式不再记录首方 `landing_view`；匿名或非管理员跳转至登录页或无权限页时也不会污染获客归因数据。
- 私有 shell 缺失时内部路径返回无统计内容的 fail-closed 503；普通公开页面继续保留既有统计注入、SEO 和获客归因行为。

## 2.9.0 - 2026-09-16

### 管理员获客漏斗

- 新增仅管理员可直接访问的独立获客漏斗页面，复用现有聚合查询接口，展示获客阶段计数、转化率、归因覆盖范围及数据完整性。
- 支持 UTC 日期范围和来源、活动、模型筛选；区分真实零值与不可用数据，并提供查询失败后的重试。
- 页面不加入导航或公开文档索引；新增文案仅覆盖英语和简体中文，其他语言沿用现有英文回退。
...
- 将 GLM-5.3 Flash 的 OpenRouter 静态参考价格更新为输入 $0.15、输出 $0.50、缓存读取 $0.03，与页面固定的 20% 节省口径保持一致。
- 移除 GLM API 价格表下方的历史核验日期和促销免责声明，保留简明的 20% 对比说明。
- 保持 Vancine 价格继续从现有 Pricing 数据动态读取，不修改模型倍率、计费逻辑、数据库或后端接口。

## 2.7.0 - 2026-09-11

### 推广落地页实时价格

- Kimi K3、GLM-5.3 与 OpenRouter Alternative 推广页改为读取现有 Pricing 数据，Vancine 当前价格不再由页面静态写死。
- 统一按实时模型倍率、完成倍率与可用分组倍率展示 USD/1M tokens；加载失败、模型缺失或特殊计费模式不会回退到旧静态价格。
- 保留带日期和来源的第三方参考价格；20% 折扣标识继续采用已确认的固定营销口径，不由前端根据实时价格反算。
...
- 为当前公开的图片与视频模型增加独立详情页，展示准确的调用端点、请求参数、支持模式、限制条件、示例代码和官方核验来源。
- 图片模型参数以生产图片生成契约为准；视频模型参数复用视频创作台的实际能力表和请求序列化逻辑，避免文档示例与真实调用格式不一致。
- 视频示例明确区分提交回执与任务轮询响应，统一使用顶层 id 轮询，并按实际 TaskStatus 展示任务状态、结果地址和失败原因。

### 动态模型目录

- 图片与视频文档总览基于实时 Pricing 数据展示当前在线模型，不再把静态注册表误认为线上可用模型清单。
- 为加载中、空列表和请求失败提供明确状态；已核验模型可进入独立详情页，未登记模型仍可在动态在线列表中显示。
- 图片与视频总览使用跨模型通用的最小请求示例，模型详情页提供对应模型的精确参数示例。

### 文档搜索与 SEO

- 将图片和视频模型详情页加入文档导航、站内搜索、服务端 metadata、sitemap 和 llms.txt。
- 为已知模型提供固定 canonical、Open Graph 和 Twitter metadata；未知模型地址返回本地化 Not Found 页面并设置 noindex。
- 增加 metadata 路径唯一性、sitemap 与 llms.txt 一致性、未知地址处理和七语言内容的回归测试。

## 2.4.1 - 2026-09-10

### 支付订单

- 将 Waffo Pancake 超时未支付订单纳入现有过期订单清理器，与 PayPal 一致在超过一小时后标记为过期，避免订单长期停留在待支付状态。
- 保持其他支付渠道、支付回调、结算流程和数据库结构不变。

## 2.4.0 - 2026-09-09

### Kimi K3 API 落地页

- 将 Kimi K3 API 页面重构为面向高意向搜索流量的价格与接入落地页，提供三方价格对比、OpenAI 兼容调用示例、已发布的历史实测证据，以及按登录状态区分的注册或控制台入口。
- 为页面补充独立 SEO metadata、固定规范链接、七语言内容和登录状态感知的转化路径。
- 明确 Vancine 是独立第三方 API 平台，不属于 Moonshot AI 或 Kimi 官方服务，与 Moonshot AI、Kimi、OpenRouter 之间不存在官方合作关系。
- 证据区仅呈现既有的一次历史受控运行与兼容性探测结果，不构成模型性能评测，也不代表未来请求的结果。

### 价格对比

- 以 OpenRouter Models API 在默认条件下返回的标准价格和 Kimi 官方公开价格为比较基准；OpenRouter 模型详情页的提供商动态价格不属于该口径，页面也不声称其固定不变。
- Kimi K3 展示价格统一为 Vancine 每百万 tokens 输入 $2.40、输出 $12.00，相比 OpenRouter 与 Kimi 官方的 $3.00、$15.00，输入与输出均低 20%。
- 页面同时给出快照日期、计价口径与排除项（免费变体、促销价格、缓存输入价格和临时提供商折扣），并说明当前结算价格以 Vancine 实时 Pricing 为准。
- 更新 OpenRouter Alternative 页面的 Kimi K3 对比行、验证日期和价格证据来源，四个旗舰模型统一引用 OpenRouter Models API。

### 国际化与验证

- 完成 Kimi K3 落地页及相关价格说明的七语言支持（简体中文、繁体中文、英语、法语、俄语、日语、越南语）。
- 增加价格快照、来源链接、CTA 生命周期、SEO metadata 和响应式页面的回归测试。

## 2.3.0 - 2026-09-08

### 动态公开模型目录

- Docs 模型页基于平台实时定价数据动态展示当前可用的文本、图片和视频模型，并按端点能力分类。
- 移除公开模型页的静态模型回退，使模型内容和数量跟随平台实际可用配置。
- 为目录加载、空数据和失败状态提供明确反馈及可控重试。

### AI Media API

- AI Media API 页面动态展示当前可用的图片和视频模型，并使用动态模型名生成图片及异步视频调用示例。
- 将公开媒体能力区域重构为图片与视频双卡片布局，提供实时 Pricing 和对应 API 文档入口。
- 单次目录重试只发起一次定价请求，避免重复请求。

### 公开内容

- 首页与 About 初始化内容收敛为当前实际提供的文本、图片和视频能力，不再承诺已经下架的 TTS 或 3D 能力。

## 2.2.1 - 2026-09-07

### 图片生成页面

- 修复图片生成页面快捷参数区域的纵向错位，统一尺寸与生成数量标签，并让尺寸、生成数量、高级设置和生成按钮保持一致的底部对齐。
- 保持移动端紧凑标签与响应式换行，不改变图片模型能力、生成参数、请求体或提交行为。

## 2.2.0 - 2026-09-07

### 视频生成创作台

- 将视频生成页面重构为与 Vancine 控制台视觉体系一致的响应式双栏创作台，集中展示生成表单、主预览和最近任务。
- 统一 API Key、视频模型、参考图片、提示词、秒数和分辨率选择，移除页面中的高级生成参数、批量数量、取消按钮及 URL 素材输入。
- 参考图片支持文件选择、拖拽和剪贴板粘贴，并按模型限制图片数量、格式、大小和请求体体积。

### 视频模型能力

- 为 wan3.0-video、wan3.0-video-prime、MiniMax-H3、Doubao-Seedance-2.0 和 Doubao-Seedance-2.5 提供明确的秒数、分辨率和参考图能力配置。
- MiniMax-H3 最多接受 5 张免费参考图片；未知视频模型继续使用仅包含模型与提示词的安全默认请求。
- 视频请求按各任务插件的既有协议生成，不修改任务插件、后端任务协议或计费逻辑。

### 任务与结果体验

- 新提交任务自动进入主预览，已提交任务在切换模型或 API Key 后继续使用提交时的密钥轮询。
- 最近任务保留最近 6 项及全部未完成任务，支持在任务之间切换预览和再次使用已提交设置。
- 视频预览和下载继续只使用平台生成的 Artifact capability URL，不公开上游原始结果地址。

## 2.1.0 - 2026-09-06

### 视频模型与任务插件

- Alibaba 内置任务插件增加 wan3.0-video 和 wan3.0-video-prime 支持。
- 保持插件身份、任务提交、轮询、计费与 Artifact 协议一致。
- Pricing 只公开启用渠道中实际可调用的插件视频模型。

### 视频生成页面

- 视频生成页面根据当前 API Key 实际可用的视频模型动态生成模型选项。
- 保留 Seedance 专用参数体验；其他视频模型在没有专用表单时使用安全的通用生成方案。
- 支持通用文本生成视频及参考图生成视频，并保持统一任务队列、轮询和结果展示。

### 公开能力调整

- 从公开文档、公开模型清单、营销页面及 SEO 数据中撤下 TTS 和 3D 能力说明。
- 移除 TTS/3D 文档导航、站内搜索和公开入口；原专用文档地址进入既有 404。
- 本次不删除后端适配器、数据库结构或管理后台配置能力；线上模型和渠道配置由管理员在发布后处理。

## 2.0.0 - 2026-09-05

### 上游主分支与控制台

- 完整接入固定上游主分支，同步上游控制台、后端、relaykit、任务协议及数据库结构变更。
- 渠道、模型、模型部署、Prefill Group、Usage Logs、账号安全和系统设置等非 Vancine 定制区域以上游实现为准。
- 产品层继续使用 Vancine 品牌、导航与营销展示；法律层保留上游 AGPL-3.0 许可证、NOTICE、第三方署名以及必须可见的上游开源归属。

### 任务插件架构

- 异步图片、视频及其他任务能力切换到上游任务插件架构：内置任务渠道通过插件注册、路由、配置及持久化任务身份工作。
- 不再保留永久 Legacy/Plugin 双栈；旧版 Legacy 任务适配代码不再作为兼容层继续维护。正式发布前必须排空仍在执行的旧版异步任务。
- 同步任务结果、Artifact、预览、下载以及任务访问控制。视频生成页按任务提交时的 API Key 查询状态与 Artifact，并校验可播放视频结果。

### 计费与额度

- 接入上游表达式计费、阶梯/动态计费和异步任务用量计费；Pricing 可展示表达式、阶梯条件及按秒等实际用量维度。
- 钱包、日志、Token、充值及相关额度字段跟随上游 64 位额度契约。
- 同步订阅余额、预扣、结算、退款以及额度边界保护，避免异常用量产生负向计费。

### Vancine 保留能力

- 保留 Vancine 首页、营销页面、SEO、品牌视觉和导航。
- 保留 Vancine 法律归属，以及必须保留的上游 AGPL、NOTICE 与第三方署名。
- 保留 PayPal 支付，以及首次充值赠送额度的展示、结算和退款扣回逻辑。
- 保留 Google OAuth 定制登录。
- 保留 Vancine 图片生成页面与视频生成页面。
- 继续兼容已接入的真实在线支付渠道：EPay/微信支付、Stripe、Creem、PayPal、Waffo 与 Waffo Pancake。手动补单、订阅入账、兑换码和后台调额不属于首次充值赠送范围。

### 升级注意事项

- 这是包含架构和数据模型变化的 2.0.0 版本，升级前必须完成数据库备份。
- 正式发布前必须排空旧版本正在执行的异步任务；旧 Legacy 任务适配代码不会作为永久兼容双栈继续维护。
- 发布使用应用级精确 SHA 部署与回滚，不操作或重启 PostgreSQL、Redis。

## 1.14.0 - 2026-09-03

### 充值与额度

- 新增“首次充值赠送 Credits”站内宣发：公共宣传、注册页与 Wallet 按后端配置动态展示赠送 Credits 数量及对应的 API balance 美元价值。
- 宣传内容由 QuotaForFirstTopUp 配置驱动；配置为 0 或配置无效（负数、超出额度上限）时，所有活动宣传与充值奖励提示自动隐藏。
- 公共状态接口新增首次充值奖励活动配置与活动状态字段；登录用户充值信息接口额外返回实际奖励额度及当前账号资格。
- 首页、注册页、定价页和相关产品宣传页面统一展示首次充值奖励，并明确“首次成功充值后到账、每个账号限一次”等条件；注册本身不赠送额度。
- Wallet 仅对符合资格的用户展示奖励说明；支付确认弹窗新增独立的奖励行与结算结果免责说明，不改变 You Pay 与实际支付金额。
- 首充资格展示与结算事务复用同一套真实充值历史判定，保持 1.13.0 已交付的渠道结算、防重复赠送、退款扣回与并发幂等逻辑不变。
- 补齐七种前端语言文案、公共页面接线测试、Wallet 数据源回归测试、状态与充值信息接口测试及资格边界测试。

## 1.13.0 - 2026-09-03

### 充值与额度

- 新增“首次充值赠送配额”配置；配置为 0 时关闭活动，启用后仅在用户首次成功完成真实充值时发放赠送配额。
- 首次充值资格在不同支付渠道之间统一判断；历史已充值用户、并发支付回调及重复回调均不会重复获得赠送。
- 退款会同时扣回充值所得配额和对应的首次充值赠送配额；已消费赠送额度时允许账户余额降为负数。
- 充值与退款后的额度缓存采用原子增减并提供失效兜底，兼容批量额度更新模式。
- 管理后台额度设置页面新增首次充值赠送配置，并完成全部七种界面语言适配。

## 1.12.0 - 2026-09-02

### 首页品牌与获客

- 重新设计首页品牌首屏，加入与 Vancine 品牌配色一致的动态 Light Tunnel、分区场景过渡及浅色与深色主题适配。
- Light Tunnel 支持减少动态效果偏好，并在页面隐藏或首屏离开视口后暂停持续渲染。
- 新增 OpenCode、Pi Agent、OpenAI SDK、Python、JavaScript 与 cURL 等接入入口，同时保留动态模型展示和现有导航结构。
- 首页、Kimi K3 与 Seedance 页面统一改为预付费接入表述，不再宣传当前已停用的注册赠送额度。
- 更新快速开始文档和相关获客材料，明确创建账号后需先充值，再使用 API Key 发起调用。

### 运营统计

- 新增公开 `GET /api/homepage/stats`，为首页提供近 30 天成功请求、处理 Token、活跃模型厂商与可用模型数量。
- 统计仅公开聚合数字，不暴露用户、密钥、渠道或账单明细；成功异步任务可以计入请求统计。
- 增加 Redis 与内存缓存、并发刷新合并、查询超时、失败退避和旧快照保留，数据库异常时不伪造零值。
- 修正 PostgreSQL 大 Token 计数的整数溢出风险，并补充统计口径、缓存和异常路径测试。

### 前端质量

- WebGL、CSS fallback 与场景过渡统一复用 Vancine 品牌主题 token。
- 增加动画生命周期、减少动态效果、预付费文案、CTA 路由、UTM 过滤和七语言文案回归测试。
- 补充 OGL 与视觉参考实现所需的第三方许可说明。

## 1.11.0 - 2026-09-01

### 模型与获客

- 首页升级为旗舰模型与高速推理模型并重的获客定位。
- 首页 Available now 继续根据实时 featured 模型动态生成，不固定模型 ID。
- 新增根据 fast 标签动态生成的高速模型展示，后续模型调整无需修改前端白名单。
- 快速模型专题页改为使用实时模型目录、价格与 fast 标签。
- 首页重新设计为对齐的双栏首屏和三段式兼容性信息轨道。
- 保留 OpenAI 兼容 API、SDK、客户端与 Agent 接入入口。
- 更新相关 SEO、导航和七语言文案。
...
- Agent 接入中心新增 Vancine 维护的 Pi Provider 安装与 /login、/model 快速接入流程。
- 增加 npm 与 GitHub 公开来源链接。
- 说明模型目录动态加载与缓存语义。
- 明确这是社区扩展，不构成 Pi 或 Earendil Works 的官方合作、认证或背书。
