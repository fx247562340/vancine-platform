	...
	for _, locale := range []struct{ language, message string }{
		...
		for _, providerCount := range []int{1, 2} {
			t.Run(fmt.Sprintf("%s/%d_providers", locale.language, providerCount), func(t *testing.T) {
				...
				// Assert the product error-type constant rather than a literal: this
				// deployment brands that value, so a hardcoded string drifts.
				assert.JSONEq(t, fmt.Sprintf(`{"error":{"message":%q,"type":%q,"code":"model_not_found"}}`,
					locale.message+" (request id: "+requestID+")", string(types.ErrorTypeNewAPIError)), recorder.Body.String())
